...
module "cloud_connector" {
  ...
  sns_topic_arns = var.cloud_trail_sns_topics
...
module "cloud_scanning" {
  count             = local.deploy_cloud_scanning ? 1 : 0
  depends_on        = [module.ecs_fargate_cluster, module.scanning_codebuild, aws_ssm_parameter.secure_endpoint, aws_ssm_parameter.secure_api_token]
