...
data "aws_iam_policy_document" "cloud_vision_role_trusted" {
  statement {
    ...
    principals {
      type = "AWS"
      identifiers = [
        "arn:aws:iam::${var.org_cloudvision_account_id}:role/OrganizationAccountAccessRole",
        "arn:aws:iam::${var.org_cloudvision_account_id}:role/sysdig-cloudvision-cloudconnector-ECSTaskRole" // FIXME. is this required or just service:ecs-task?
      ]
    }
